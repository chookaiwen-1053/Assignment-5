...
Lastly, the project focuses on the development and evaluation of a predictive model. By building a Linear Regression model, the goal is to forecast electricity costs (Cost_RM) using consumption levels and occupancy as independent variables. The objective includes a formal evaluation phase where the model's performance is measured through R2 and Mean Absolute Error (MAE). This ensures that the findings are not just descriptive but also possess predictive utility for future scenarios.
